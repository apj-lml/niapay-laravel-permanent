<div>
    <font size="1.75" face="Cambria" >
      ...
      <div class="modal fade" id="showPayslipModal" tabindex="-1" aria-labelledby="showPayslipModalLabel" aria-hidden="true" wire:ignore.self>
          <div class="modal-dialog modal-fullscreen">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="showPayslipModalLabel">Download Payslip</h5>
                            ...
                            <div class="accordion-item">
                              <h2 class="accordion-header" id="flush-heading{{ $year }}{{ $month }}">
                                ...
                                <button class="accordion-button collapsed fs-6 fw-lighter" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapse{{ $year }}{{ $month }}" aria-expanded="false" aria-controls="flush-collapseOne">
                                  {{ $month }} <a class="fs-6 fw-lighter" href="#" onclick="return false;" style="z-index: 999;" wire:click="downloadPayslipByMonth('{{ $month }}')"> <i class="bi bi-download ms-1"></i></a>
                                </button>
                              </h2>
                              <div id="flush-collapse{{ $year }}{{ $month }}" class="accordion-collapse collapse" aria-labelledby="flush-headingOne" data-bs-parent="#accordionFlushMonths">
                                  <div class="accordion-body">
                                    @foreach ($payslips as $payslip)
                                        <p><a href="#" wire:click="downloadPayslip({{ $payslip->id }}, {{ $payslip->period_covered_from }}, {{ $payslip->period_covered_to }})">{{ $payslip->period_covered }}</a></p>
                                    @endforeach
                                  </div>
                              </div>
                            </div>
