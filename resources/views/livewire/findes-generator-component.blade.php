<div>
    <div wire:loading.delay.short>
      <div id="spinner" class="show bg-white position-fixed translate-middle w-100 vh-100 top-50 start-50 d-flex align-items-center justify-content-center">
          ...
          <p class="p-0 m-0 ps-3">Generating remittances... It may take a while!</p>
      </div>
    </div>
      <div class="row wow fadeInUp" data-wow-delay="0.1s">
        <div class="col-md-6">
          <div class="form-floating mb-3 wow fadeInUp">
              <select class="form-select @error('frequency') is-invalid @enderror" id="frequency" aria-label="frequency" wire:model="frequency">
                  {{-- <option value="all" >All</option> --}}
                  <option value="full_month" selected>Full Month</option>
                  <option value="first_half">First Half</option>
                  <option value="second_half">Second Half</option>
              </select>
              <label for="floatingSelect">Select Payroll Period</label>
              @error('frequency')
      ...
      <div class="col-md-6">
        <div class="form-floating mb-3 wow fadeInUp">
            <select class="form-select @error('isLessFifteen') is-invalid @enderror" id="isLessFifteen" aria-label="isLessFifteen" wire:model="isLessFifteen">
                {{-- <option value="all" >All</option> --}}
                <option value="0" selected>Above 15 Leave Credits</option>
                <option value="1">Below 15 Leave Credits</option>
            </select>
            <label for="floatingSelect">Select Payroll Period</label>
            @error('isLessFifteen')
                <span class="invalid-feedback" role="alert">
      ...
      <div class="text-center mt-3">
        <button class="btn btn-primary" wire:click="generateFindes()">Generate FINDES</button>
      </div>
  </div>
  ...
  function setDates(currentDate, fifteenthDay){
        ...
        document.getElementById('dateToPayroll').value = dayjs().endOf('month').format('YYYY-MM-DD')
